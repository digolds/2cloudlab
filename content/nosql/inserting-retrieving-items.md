...
date: 2019-02-23T12:27:38+06:00
...
type: post
image: images/blog/inserting-retrieving-items.png
author: Alex
tags: ["NoSQL", "DynamoDB", "Data-Intensive"]
---

[数据项](https://2cloudlab.com/nosql/anatomy-of-an-item/)是DynamoDB的基础单元，每一张表都会包含多项数据。接下来，在本文中，我们将向DynamoDB中插入和读取数据项。我们将创建Users表，并为该表指定一个[简单键](https://2cloudlab.com/nosql/anatomy-of-an-item/)：Username。接着，我们将操作2个基本的接口：PutItem和GetItem。下一篇文章，我们将在这篇文章的基础上[应用表达式](https://2cloudlab.com/nosql/expression-basics/)来实现更加复杂的查询功能。在这之后，我们将另起一篇文章来讲解[如何更新和删除数据项](https://2cloudlab.com/nosql/updating-deleting-items)。

>为了能顺利操作本文所列举的示例，请确保[DynamoDB的环境](https://2cloudlab.com/nosql/environment-setup)已经准备好。注意：如果你使用的是本地版本的DynamoDB，那么请确保`$LOCAL`变量配置正确，如果使用的是AWS上的DynamoDB，那么在每一个命令后面无需追加这个参数。
...
>如果你想避免覆盖原有的数据项，那么可以考虑使用[条件表达式](https://2cloudlab.com/nosql/expression-basics/)。如果你只是想更新某项数据的某些属性，那么你可以考虑使用DynamoDB的[UpdateItem API](https://2cloudlab.com/nosql/updating-deleting-items/)。
...
注意：在上述操作示例中，我们不仅提供了表"Users"所必须的属性"Username"，我们还提供了其它非必须的属性，比如"Name"和"Age"以及每一个属性[对应的类型]((https://2cloudlab.com/nosql/anatomy-of-an-item/)。与关系型数据库不同的是，DynamoDB允许开发者在添加数据项时决定具体的属性信息（Schema-Less），而不是通过Schema提前定义好数据类型。
...
这篇文章涉及了向DynamoDB中插入和获取单项数据的基础操作示例。在接下来的[一篇文章](https://2cloudlab.com/nosql/expression-basics/)中，我们将结合表达式来实现更加复杂实用的插入和查询操作。
